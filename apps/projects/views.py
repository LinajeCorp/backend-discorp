...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        ...
        responses={200: ProjectSerializer, 404: "Proyecto no encontrado"},
        tags=["Proyectos"],
    )
    ...
    @swagger_auto_schema(
        ...
        responses={201: ProjectSerializer, 400: "Datos inválidos"},
        tags=["Proyectos"],
    )
        ...
        responses={
            200: openapi.Response(
                ...
                examples={
                    "application/json": {
                        "total_proyectos": 12,
                        "por_fase": {"ejecucion": 5, "completado": 6, "pausado": 1},
                        "por_status": {
                            "en_curso": 4,
                            "en_seguimiento": 6,
                            "pausado": 2,
                        },
                        "por_prioridad": {"normal": 8, "alta": 4, "baja": 0},
    ...
    def estadisticas(self, request):
        ...
        por_fase = {}
        for fase_value, _ in Project.FASE_CHOICES:
            por_fase[fase_value] = Project.objects.filter(fase=fase_value).count()

        # Estadísticas por status
        por_status = {}
        for status_value, _ in Project.STATUS_CHOICES:
            por_status[status_value] = Project.objects.filter(
                status=status_value
            ).count()

        # Estadísticas por prioridad
        por_prioridad = {}
        for prioridad_value, _ in Project.PRIORIDAD_CHOICES:
            por_prioridad[prioridad_value] = Project.objects.filter(
